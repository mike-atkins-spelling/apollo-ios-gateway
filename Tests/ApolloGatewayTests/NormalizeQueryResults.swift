...
import ApolloGatewayTestSupport
import StarWarsAPI
...
class NormalizeQueryResults: XCTestCase {
  func testHeroNameQuery() throws {
    ...
    XCTAssertEqual(records?["QUERY_ROOT"]?["hero"] as? CacheReference, CacheReference(key: "QUERY_ROOT.hero"))
  ...
  func testHeroNameQueryWithVariable() throws {
    ...
    XCTAssertEqual(records?["QUERY_ROOT"]?["hero(episode:JEDI)"] as? CacheReference, CacheReference(key: "QUERY_ROOT.hero(episode:JEDI)"))
  ...
  func testHeroAndFriendsNamesQueryWithoutIDs() throws {
    ...
    let hero = try XCTUnwrap(records?["QUERY_ROOT.hero(episode:JEDI)"])
    XCTAssertEqual(hero["name"] as? String, "R2-D2")
    XCTAssertEqual(hero["friends"] as? [CacheReference], [CacheReference(key: "QUERY_ROOT.hero(episode:JEDI).friends.0"), CacheReference(key: "QUERY_ROOT.hero(episode:JEDI).friends.1"), CacheReference(key: "QUERY_ROOT.hero(episode:JEDI).friends.2")])
  ...
  func testHeroAndFriendsNamesQueryWithIDs() throws {
    ...
    XCTAssertEqual(records?["QUERY_ROOT"]?["hero(episode:JEDI)"] as? CacheReference, CacheReference(key: "2001"))
    
    let hero = try XCTUnwrap(records?["2001"])
    XCTAssertEqual(hero["name"] as? String, "R2-D2")
    XCTAssertEqual(hero["friends"] as? [CacheReference], [CacheReference(key: "1000"), CacheReference(key: "1002"), CacheReference(key: "1003")])
  ...
  func testHeroAndFriendsNamesQueryWithIDForParentOnly() throws {
    ...
    let hero = try XCTUnwrap(records?["2001"])
    XCTAssertEqual(hero["name"] as? String, "R2-D2")
    XCTAssertEqual(hero["friends"] as? [CacheReference], [CacheReference(key: "2001.friends.0"), CacheReference(key: "2001.friends.1"), CacheReference(key: "2001.friends.2")])
